...
class TVWiki : MainAPI() {
    ...
    private fun Element.toSearchResponse(): SearchResponse? {
        ...
        val title = this.selectFirst("a.title")?.text()?.trim() ?: this.selectFirst("a.title2")?.text()?.trim() ?: return null
        ...
        return when (type) {
            ...
            TvType.Anime -> newAnimeSearchResponse(title, link, TvType.Anime) { this.posterUrl = fixedPoster }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        if (items.isEmpty()) { items = doc.select("#list_type ul li").mapNotNull { it.toSearchResponse() } }
        return items
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val genreList = doc.select(".tags dd a").filter { !it.text().contains("트레일러") && !it.hasClass("btn_watch") }.map { it.text().trim() }
        ...
        val castFormatted = if (castList.isNotEmpty() && castList.none { it.contains("운영팀") }) "출연: ${castList.joinToString(", ")}" else ""
        ...
        var story = doc.selectFirst("#bo_v_con")?.text()?.trim() ?: doc.selectFirst(".story")?.text()?.trim() ?: doc.selectFirst("meta[name='description']")?.attr("content") ?: ""
        if (story.contains("다시보기") && story.contains("무료")) story = "다시보기"
        if (story.isEmpty()) story = "다시보기"
        val finalPlot = if (story == "다시보기") "다시보기" else if (metaString.isNullOrBlank()) "줄거리: $story".trim() else "$metaString / 줄거리: $story".trim()
        ...
        val episodes = doc.select("#other_list ul li").mapNotNull { li ->
            ...
            val epThumb = thumbImg?.attr("data-src")?.ifEmpty { null } ?: thumbImg?.attr("data-original")?.ifEmpty { null } ?: thumbImg?.attr("src")?.ifEmpty { null } ?: li.selectFirst("img")?.attr("src")
            newEpisode(href) {
                this.name = epName
    ...
    private suspend fun findAndExtract(
        ...
        if (iframe == null) iframe = doc.selectFirst("iframe[src*='bunny-frame']")
        ...
        for (script in scriptTags) {
            ...
            if (scriptContent.contains("player.bunny-frame.online")) {
                val match = Regex("""https://player\.bunny-frame\.online/[^"'\s]+""").find(scriptContent)
                if (match != null) {
