...
class TVHot : MainAPI() {
        ...
        override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
                ...
                doc.select("section").forEach { section ->
                        ...
                        // 각 섹션의 아이템 가운데 2개만 가져오기
                        val items = section.select(".owl-carousel .item").mapNotNull { it.toSearchResponse() }
                        val limited = items.take(2) // 여기서 2개로 제한

                        if (limited.isNotEmpty()) home.add(HomePageList(title, limited))
                }
